...
<body class="min-h-screen bg-gradient-to-br from-[#0b1020] via-[#1a1a2e] to-[#16213e] flex items-center justify-center p-4 font-sans antialiased">
    ...
    <div class="w-full max-w-md relative z-10" x-data="registerForm()">
        <!-- Card Container -->
            ...
            <form @submit.prevent="submitForm" class="space-y-5">
                    ...
                    <div x-show="password.length > 0" x-cloak class="space-y-2">
                        <div class="flex gap-1">
                            <div class="h-1 flex-1 rounded-full transition-all duration-300"
                                 :class="passwordStrength >= 1 ? (passwordStrength >= 3 ? 'bg-emerald-500' : passwordStrength >= 2 ? 'bg-yellow-500' : 'bg-red-500') : 'bg-white/10'"></div>
                            <div class="h-1 flex-1 rounded-full transition-all duration-300"
                                 :class="passwordStrength >= 2 ? (passwordStrength >= 3 ? 'bg-emerald-500' : 'bg-yellow-500') : 'bg-white/10'"></div>
                            <div class="h-1 flex-1 rounded-full transition-all duration-300"
                                 :class="passwordStrength >= 3 ? 'bg-emerald-500' : 'bg-white/10'"></div>
                            <div class="h-1 flex-1 rounded-full transition-all duration-300"
                                 :class="passwordStrength >= 4 ? 'bg-emerald-500' : 'bg-white/10'"></div>
                        </div>
                        <p class="text-xs" :class="passwordStrength >= 3 ? 'text-emerald-400' : passwordStrength >= 2 ? 'text-yellow-400' : 'text-red-400'" x-text="passwordStrengthText"></p>
                    </div>
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <div class="relative">
                        ...
                        <button type="button" @click="showConfirmPassword = !showConfirmPassword" class="absolute inset-y-0 right-0 pr-4 flex items-center">
                            <svg x-show="!showConfirmPassword" class="w-5 h-5 text-white/40 hover:text-white/60 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/>
                            </svg>
                            <svg x-show="showConfirmPassword" x-cloak class="w-5 h-5 text-white/40 hover:text-white/60 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.88 9.88l-3.29-3.29m7.532 7.532l3.29 3.29M3 3l3.59 3.59m0 0A9.953 9.953 0 0112 5c4.478 0 8.268 2.943 9.543 7a10.025 10.025 0 01-4.132 5.411m0 0L21 21"/>
                            </svg>
                        </button>
                    </div>
                    <!-- Password Match Indicator -->
                    <p x-show="confirmPassword.length > 0" x-cloak class="text-xs flex items-center gap-1.5"
                       :class="password === confirmPassword ? 'text-emerald-400' : 'text-red-400'">
                        <svg x-show="password === confirmPassword" class="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 20 20">
                            <path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"/>
                        </svg>
                        <svg x-show="password !== confirmPassword" x-cloak class="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 20 20">
                ...
                <label class="flex items-start gap-3 cursor-pointer group">
                    <div class="relative mt-0.5">
                        <input type="checkbox" x-model="agreeTerms" class="sr-only peer" required>
